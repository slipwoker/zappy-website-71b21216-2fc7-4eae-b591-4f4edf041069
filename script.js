    ...
    contactForm.addEventListener('submit', async function(e) {
      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const message = messageField ? messageField.value : `שם: ${name}\nטלפון: ${phone}\nאימייל: ${email}`;
          
          // Extract subject field if exists
          const subjectField = document.getElementById('subject');
          const subject = subjectField ? subjectField.value : 'פנייה חדשה מטופס יצירת קשר';
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '71b21216-2fc7-4eae-b591-4f4edf041069',
              name: name,
              email: email,
              subject: subject,
              message: message,
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שירה ניילס משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
